...
- **Dashboard data shape** (`DashboardData` in `app/lib/dashboard/types.ts`) carries two cash-flow series: `trend` (12 monthly points) and `dailyTrend` (current month, one point per day, day 1 → today). `loadDashboardData` buckets `dailyTrend` from the already-fetched current-month transactions (no extra query); keep `sample.ts` in sync (it generates a deterministic daily series). `SpendingTrend` (`app/dashboard/_components/spending-trend.tsx`) has a **12M / MTD range toggle** that switches between the two, on top of the existing Expense/Income/Net series toggle.
...
- **Always format money through `formatCurrency` (`app/lib/format.ts`)** — never hardcode a currency symbol. It renders the **complete amount with thousands grouping (no "K"/"M" compact notation)**: whole values show no decimals (`₹29,397`), fractional values show 2 (`₹42.20`). Pass `{ sign: true }` to force a leading `+`/`−`. The symbol comes from the passed `currency` (the signed-in user's `User.currency`, surfaced as `data.user.currency` on the dashboard).
- Animated KPI counters use `AnimatedNumber` (`app/dashboard/_components/motion-primitives.tsx`), which calls its `format` callback on each animation frame (and renders a plain `toLocaleString` integer when no `format` is given — used by the health score, which must stay symbol-less). KPI cards pass `(v) => formatCurrency(Math.round(v), currency)` so the count-up rolls through whole amounts without flickering decimals.
